...
<body>
    ...
    <div class="container">
                        ...
                        if (property_exists($jdata, "pumps")) {
                            foreach ($jdata->pumps as $a) {
                                ...
                                echo <<<END
                                            ...
                                            <form method="POST">
                                                <input type="text" name="$name" value="0" readonly hidden/>
                                                <input type="submit" class="btn btn-success" value="Ein"/>
                                            </form>
                                            <form method="POST">
                                                <input type="text" name="$name" value="1" readonly hidden/>
                        ...
                        */
                        ...
                        if(isset($_POST["path"])){
                            if(strlen($_POST["path"]) > 0){
                                $path = $_POST["path"];
                                curl_setopt($ch, CURLOPT_URL, "http://heizung".$path."?" . http_build_query($_POST));
                                $result = curl_exec($ch);
                                curl_error_verbose($ch, $result);
                            }else{
        ...
        <div>
            <h3>Uptime</h3>
            <p>
                Wenn der ESP32 rebooted, dann ist das hier in der Liste. Wenn man den Grund weiß, dann kann man den
                Check
                referenzieren. Dadurch weiß man dann, dass der Reboot bekannt ist.
            </p>
            <div class="table-responsive">
                <table class="table">
                    <thead>
